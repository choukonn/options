...
class FileCtrl():
	...
	def utf8bomcheck(self, filePath):
		ret = False
	...
	def fileOpen(self, fp, enc, flagHeader):
		data = None

		try:
			with open(fp, mode='r', encoding=enc, newline=None) as fr:
			...
			self.logger.error('csv file encoding error, {}'.format(err))
			raise

		return data

	def csvRead(self, filePath, encoding='UTF-8', flagHeader=True):
		if filePath is None:
		...
		try:
			if encoding.upper() in ['UTF-8', 'utf8']:
				utf8Bomflag = self.utf8bomcheck(filePath)
				if utf8Bomflag == True: encoding = 'UTF-8-SIG'

			try:
				data = self.fileOpen(filePath, encoding, flagHeader)
			except UnicodeDecodeError:
				# TODO: windows相手ならもうcp932固定でいいよね
				tmpEnc = ['UTF-8', 'cp932']
				if encoding.upper() in tmpEnc:
					tmpEnc.remove(encoding)
					if len(tmpEnc) == 1:
						tmpEnc = tmpEnc[0]
					else:
						raise
				try:
					# エンコードエラーになったら、固定でcp932でファイルオープンを試す
					self.logger.info('Re-execute with character code {}'.format(tmpEnc))
					data = self.fileOpen(filePath, tmpEnc, flagHeader)
					self.logger.info('Successful re-execution with character code {}'.format(tmpEnc))
				except:
					raise
	...
	# 気に入らないから保留
	#def csvReadNoHeader(self, filePath, encoding=None):
	#	if filePath is None:
	#		return
	#
	#	try:
	...
	def csvWrite(self, filePath, data, header, encoding='UTF-8'):
		if filePath is None:
			return
		if header is None:
			return
	...
	def xmlRead(self, filePath, encoding=None):
		if filePath is None:
			return
		...
		try:
			...
			self.logger.error('xml file encoding error, {}'.format(err))
			return
	...
	def xmlWrite(self, filePath, xmlObj):
		if filePath is None:
			return
		if xmlObj is None:
			return
	...
	def textRead(self, filePath, encoding=None):
		if filePath is None:
			self.logger.debug('args error filePath')
			raise
		...
		try:
			...
			self.logger.error('text file encoding error, {}'.format(err))
			return
	...
	def textWrite(self, filePath, data, encoding=None):
		if filePath is None:
			eMsg = 'args error filePath'
			self.logger.debug(eMsg)
			raise Exception(eMsg)
		...
		try:
			...
			with open(filePath, mode=wType, encoding=encoding, newline=None) as fw:
				if type(data) == list:
					...
					self.logger.warning('unknown type, text write failed')
	...
	def jsonRead(self, filePath, encoding=None):
		if filePath is None:
			self.logger.debug('args error filePath')
			raise
		...
		try:
			...
			self.logger.error('json file encoding error, {}'.format(err))
			return
