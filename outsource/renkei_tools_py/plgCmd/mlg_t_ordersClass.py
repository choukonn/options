...
class MlgTorders(mySql.Exceute):
	...
	# mlg_t_orders
	def setMlgPostOrder(self, sidMorg, *, sidAppoint, ssidLinkageProduct, ssidFileKind, xml, orderNo, ssidFrom, fileName, orderKey, seqNo, targetName):
		if sidMorg is None:
			return None
		...
		try:
			...
			if rows is None:
				self.logger.warning('[{sidMorg}] [p_mlg_post_order] post faild, sidAppoint: {sidAppoint}, orderKey: {orderkey}'.format(sidMorg=sidMorg, sidAppoint=sidAppoint, orderkey=orderKey))
				return None
		except Exception as err:
	...
	# 受診者情報の更新
	def setMlgPostExam(self, sidMorg, *, ssidLinkageProduct, ssidFileKind, xml, orderNo, ssidFrom, fileName, orderKey):
		if sidMorg is None:
			return None
		...
		try:
			...
			rows = self.once(query, param)
			if rows is None:
				self.logger.warning('[{sidMorg}] [p_mlg_post_exam] post faild'.format(sidMorg=sidMorg,))
				return None
		except Exception as err:
			self.logger.debug(err)
			raise
