...
class TappointMe(mySql.Exceute):
	...
	# t_appoint_me
	# XMLMEの部分更新用ストアド
	def setUpdateXMLME(self, sidMorg, *, sidAppoint, elementSidExam, elementVal=None, eitemSidExam=None, valueForm=None, f_intendedOnly=None, qualCode=None):
		if sidMorg is None:
			return None
		...
		# IN qualCode VARCHAR(10)				-- 8:定性値用のコード

		try:
			#                                     1  2  3  4  5  6  7  8  9
			query = 'CALL p_updateXmlMeElementVal(?, ?, ?, ?, ?, ?, ?, ?, ?);'
			param = (sidMorg, systemUserSid, sidAppoint, elementSidExam, elementVal, eitemSidExam, valueForm, f_intendedOnly, qualCode)
	...
	# 検査項目の基準SID更新用（便潜血など1回法、2回法で基準の切り替えが必要な場合に使用）
	def setUpdateXMLMEsidCriteion(self, sidMorg, *, sidAppoint, eitemSidExam=None, sidCriterion=None):
		if sidMorg is None:
			return None

		try:
			query = " update t_appoint_me set xml_me = updatexml(xml_me, CONCAT('//eitem[sid[text()=\"', ?,'\"]]/sid_criterion'), concat('<sid_criterion>', ?, '</sid_criterion>')) \
					  where sid_morg = ? and sid_appoint = ?; "
			param = (eitemSidExam, sidCriterion, sidMorg, sidAppoint)
			rows = self.once(query, param)
			if rows is None:
				self.logger.debug('[{sidMorg}] [XMLME] update failed found [sidAppoint: {sidApo}, eitemSidExam:{isid}, sidCriterion:{sidCr}]'.format(sidMorg=sidMorg, sidApo=sidAppoint, isid=eitemSidExam, sidCr=sidCriterion))
				return None
		except Exception as err:
			self.logger.debug(err)
			raise
	...
	# XMLMEの登録
	def createUpdateXmlMeElementResult(self, *, sidMorg=None, sidAppoint=None, data=None, in_sid_upd=None):
		...
		try:
			...
			param = (sid_upd, dtUpd, sidMorg, sidAppoint)
