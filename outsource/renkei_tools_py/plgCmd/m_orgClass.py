...
class Morg(mySql.Exceute):
	...
	# 紐づけ団体の検索
	def searchXorgId(self, sidMorg, *, sidExaminee):
		...
		try:
			...
			query = ' \
				...
				WHERE mxorg.sid_morg = ? AND mxorg.sid_examinee = ?; \
			'
	...
	# 団体登録情報更新
	def setXorgUpdate(self, sidMorg, *, xml):
		# ストアド引数
	...
	# 団体紐づけ
	def setXorgAdd(self, sidMorg, *, sidExaminee, sidOrg, fCurrent, xml):
		# ストアド引数
		# IN	IN_sid_morg		INT UNSIGNED,		1: 医療機関番号
		# IN 	IN_sid_upd		INT UNSIGNED,		2: 更新した人のsid、システムで固定
	...
	# 団体ID検索
	def searchOrgId(self, sidMorg, *, sid, orgId=None, sOrgId=None):
	...
	# 団体XMLの登録
	def setOrgXml(self, sidMorg, *, xml, sid):
		# ストアド引数
		...
		try:
			...
			if mode == 'POST':
				...
				# 団体検索
				rows = self.searchOrgId(sidMorg, sid=sid)
				# 0件は何もしない
				if len(rows) < 1:
					return None
				# 複数ヒットしたら何もしない
