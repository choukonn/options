	...
	if criterionSid is not None:
		...
		if 'f_appoint_only' in optionMap and optionMap['f_appoint_only'] == 1:
			...
			try:
				...
				outsourcingInspectionMap = m_eorg.getOutsourcingMap(sidMorg, courseXMLObj=courseXMLObj)
				# 要素コードだけ欲しい
				if outsourcingInspectionMap['eie'] is not None:
					inspCodeMap = {k: v['res'] for k, v in outsourcingInspectionMap['eie'].items() if v['res'] is not None}
				if inspCodeMap is None:
...
def updateXmlMe(sidMorg, cid, tAppoint, courseSid, data, xmlMeInfo):
	...
	# 設定対象のelementと一致するものをマッピングリストから抽出
	matchingEieSid = {k : inspCodeMap[k] for k in set(inspCodeMap) & set(criterionSid[courseSid]['ele2eitem'].keys())}
...
def setTappointMe(sidMorg, data):
	...
	# xmlMeInfo obj キャッシュする
	# {courceId: criterion obj}
	xmlMeInfo_cache = {}
	...
	for row in data:
		...
		# 予約処理モード(こうのす)
			...
			if 'appointTime' in appointMap and appointMap['appointTime'] in row:
				...
				# 分割チェック
				# re.match(r'[0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}', tmpDateTime[1])
			...
			if plgCmn.dateFormatCheck(examDate) == False:
				...
				errRowData.append(row)
				continue
			date_string = examDate + ' ' + examDateTime
			log("{}".format(date_string))
			...
			if appointMap['apoAction'] in row and int(row[appointMap['apoAction']]) == appointAct_konosu.cancel and tAppoint is None:
				log('[{sidMorg}] Cancel action, but skip because t_appoint does not exist'.format(sidMorg=sidMorg))
				msg2js('カルテID:{cid}, コース名:{courseName}, 受診日：{examDate}, {msg}'.format(cid=cid, courseName=row['CourseName'], examDate=examDate, msg='予約情報が見つからないため、予約キャンセルをスキップします。'))
				continue
			...
			if tAppoint is not None:
				...
				for app in tAppoint:
					log('inCourseID: {}'.format(app['inCourseID']))
				...
				if tmpApp is None:
					...
					log("!!!!!!!!!!!!!!!!!!!!!!")
					log('appointAct_konosu.cancel: {}'.format(appointAct_konosu.cancel))
					log('apoAction: {}'.format(row[appointMap['apoAction']]))
					log(row[appointMap['apoAction']])
					# 予約キャンセルする？
					# if appointMap['apoAction'] in row and int(row[appointMap['apoAction']]) == appointAct.cancel:
					if appointMap['apoAction'] in row and int(row[appointMap['apoAction']]) == appointAct_konosu.cancel:	
			...
			if courseMapInfo['courseId'] in xmlMeInfo_cache:
				xmlMeInfo = xmlMeInfo_cache[courseMapInfo['courseId']]
			else:
				# XMLMEや基準の取得
				start_time = time()
				xmlMeInfo = getXmlMeInfo(sidMorg, tAppoint, courseMapInfo, row, useContractInfo)
				endTime = time()
				log('time cost of xmlMeInfo: {}'.format(endTime - start_time))
				xmlMeInfo_cache[courseMapInfo['courseId']] = xmlMeInfo

				log('path: {}'.format(os.getcwd()))
				# text_file = open("xmlMeInfo.txt", "w")
				# n = text_file.write(str(str(xmlMeInfo).encode('utf8')))
				# text_file.close()
			...
			if tAppoint is None:
				...
				if 0 <= tAppoint['status'] <= 1:
					...
					"""
					msg = ''
					if appointUpdFlag == changeSts.apoDay: # 日付変更（オーダー送信テーブル側で新規予約として）
						ordSts = examSts['appoint']
						msg = '日付変更しました'
					...
					# t_appoint.status(0:予約, 1:受付)
					tApoStsCode = tAppoint['status']
					# t_appoint&t_appoint_meのレコード更新(status・apoDate 更新)
...
def main():
	...
	try:
		...
		dataJson = ""
		encoding = "shift-jis"
		with open(config_data['file_path'], 'r', encoding=encoding) as csvf:
		...
		dataJson = json.dumps(jsonArray, indent=4)
		log(type(dataJson))
		"""
		...
		json_dict = []
		dataJson = ""
		# headers = ["AppointDate","AppointNumber","AppointStatus","CourseName","CourseId","Id","Name","Kana","Sex","Dob","Age","Zip","Address","Company","CompanyId"]
		...
		# tmpfile = open(config_data['file_path'], 'r', encoding='UTF-8')
		# dataJson = tmpfile.read()
		# tmpfile.close()
	...
	try:
		...
		if dataJson is not None and len(dataJson) > 0:
			...
			try:
				...
				log('time cost of csv2json: {}'.format(endTime_csvRead - startTime_csvRead))
			except Exception as err:
